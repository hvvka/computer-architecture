...
extern char* rozmycieAsm(unsigned int, unsigned char*, unsigned char*, int);
...
int main()
{
  ...
  if (wybor == 1) {
    ...
    char *dataAsm = rozmycieAsm(Header.biWidth*3, data, newData, IMAGE_SIZE);	// przetworzenie obrazu
...
char* rozmycieC(unsigned int imageWidth, unsigned char* data, int imageSize)
{
  int counter = 0, i = 0;
  unsigned char* newData = (unsigned char*) calloc (imageSize, 1);
  int sum = 0;	     // zmienna pomocnicza do sumnewDataowania wartości dwóch pixeli
  ...
  for(i = 0; i < imageSize; i++) {
    ...
    if((i+imageWidth) < imageSize) {
      counter++;
      sum += data[i+imageWidth];
    }
    // pod
    if((i-imageWidth) > 0) {
      counter++;
    ...
    newData[i] = sum; // zapisanie uśrednionej wartości max 5 pixeli
  }
